...
|---|---|---|---|
| `id` | string | no | Stable identifier within this response, e.g. `"w0"`, `"w1"`. |
...
| `phonemes` | object[] | no | Per-phoneme breakdown. |
...
| `status` | string | no | `correct`, `mispronounced`, `omitted`, or `insertion`. |
| `tolerated` | boolean | no | `true` when a tolerance rule forgave this position rather than the speaker matching the reference. The phoneme counts as acceptable, but it was not produced as written — useful if you want to show a learner the difference without marking it wrong. A tolerated phoneme keeps `status: "correct"` while scoring low, so read the two together rather than colouring by score alone. |
...
overlap**; they are not a partition of the word. In the sample response, `test`
returns `t` → `[0,1)`, `ɛ` → `[1,3)`, `s` → `[2,3)` and `t` → `[2,4)`, so the
character at index 2 belongs to three phonemes at once. Treat each span as its
own hint about one sound, and use the word's `mispronouncedGraphemes` when you
need ranges you can paint without overlapping.

Three behaviours worth coding for:
...
- A word's spans are **not** the union of its phonemes' `graphemes`. An inserted
  sound has `graphemes: null` at the phoneme level, but if it corresponds to a
  silent letter of the reference word, the word still gains a span covering that
  letter. Pronouncing the *t* in *est* is the textbook case: the phoneme entry
  has `"graphemes": null`, while the word reports `{"start": 2, "end": 3}`.
  Always read the word's `mispronouncedGraphemes` for highlighting rather than
  assembling spans yourself.
...
Note in particular that an `insertion` — the speaker saying something extra —
is reported on its own word entry but never makes the utterance unacceptable.

`insertion` names three different things: a word `status`, a phoneme `status`,
and a value in `faults`. Only the **word** `status` bears on `accepted`. The
sample response shows why that matters: its top-level `faults` contains
`insertion` while `accepted` is `false`, because the insertion is a *phoneme*
inside `est` — which made that word `mispronounced` — and it was the omitted
`final` that made the utterance unacceptable. No word in that response has
`status: "insertion"` at all.
